...
<%= data.branding.codePrefix %>.FriendService.AddFriends(profileIds, successCallback, failureCallback);
```
...
<%= data.branding.codePrefix %>->getFriendService()->addFriends(profileIds, this);
```
...
[[<%= data.branding.codePrefix %> friendService] addFriends:profileIds
...
var postResult = friendProxy.addFriends(profileIds);
