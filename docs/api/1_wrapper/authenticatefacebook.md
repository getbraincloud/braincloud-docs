...
<%= data.branding.codePrefix %>:authenticateFacebook(facebookId, token, forceCreate, callback)
```
...
var result = await <%= data.branding.codePrefix %>.authenticate_facebook(facebook_id, token, force_create)
