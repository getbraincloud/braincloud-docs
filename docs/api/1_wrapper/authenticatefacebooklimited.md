...
ServerResponse result = await <%= data.branding.codePrefix %>.authenticateFacebookLimited(
    fbUserId:facebookId, 
    fbAuthToken:token, 
    forceCreate:forceCreate);
...
<%= data.branding.codePrefix %>:authenticateFacebookLimited(facebookId, token, forceCreate, callback)
```
