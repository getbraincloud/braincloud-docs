...
ServerResponse result = await <%= data.branding.codePrefix %>.smartSwitchAuthenticateFacebookLimited(
    fbUserId:facebookId, 
    fbAuthToken:token, 
    forceCreate:forceCreate);
...
<%= data.branding.codePrefix %>:smartSwitchAuthenticateFacebookLimited(facebookId, token, forceCreate, callback)
```
