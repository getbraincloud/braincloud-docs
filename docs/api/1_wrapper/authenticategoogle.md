...
<%= data.branding.codePrefix %>:authenticateGoogle(googleUserId, serverAuthCode, forceCreate, callback)
```
...
var result = await <%= data.branding.codePrefix %>.authenticate_google(google_user_id, server_auth_code, force_create)
