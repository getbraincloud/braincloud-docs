...
ServerResponse result = await <%= data.branding.codePrefix %>.tournamentService.leaveTournament(leaderboardId:leaderboardId);
...
<%= data.branding.codePrefix %>:getTournamentService():leaveTournament(leaderboardId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.tournament_service.leave_tournament(leaderboard_id)
