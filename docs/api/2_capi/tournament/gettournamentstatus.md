...
<%= data.branding.codePrefix %>:getTournamentService():getTournamentStatus(leaderboardId, versionId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.tournament_service.get_tournament_status(leaderboard_id, version_id)
