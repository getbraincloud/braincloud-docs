...
ServerResponse result = await <%= data.branding.codePrefix %>.tournamentService.getMyDivisions();
...
<%= data.branding.codePrefix %>:getTournamentService():getMyDivisions(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.tournament_service.get_my_divisions()
