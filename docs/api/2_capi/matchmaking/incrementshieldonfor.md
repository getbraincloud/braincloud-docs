...
<%= data.branding.codePrefix %>:getMatchMakingService():incrementShieldOnFor(minutes, callback)
```
...
var result = await <%= data.branding.codePrefix %>.match_making_service.increment_shield_on_for(minutes)
