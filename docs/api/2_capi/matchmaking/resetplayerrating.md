...
ServerResponse result = await <%= data.branding.codePrefix %>.matchMakingService.resetPlayerRating();
...
<%= data.branding.codePrefix %>:getMatchMakingService():resetPlayerRating(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.match_making_service.reset_player_rating()
