...
<%= data.branding.codePrefix %>:getMatchMakingService():decrementPlayerRating(decrement, callback)
```
...
var result = await <%= data.branding.codePrefix %>.match_making_service.decrement_player_rating(decrement)
