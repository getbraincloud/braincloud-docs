...
<%= data.branding.codePrefix %>:getMatchMakingService():incrementPlayerRating(increment, callback)
```
...
var result = await <%= data.branding.codePrefix %>.match_making_service.increment_player_rating(increment)
