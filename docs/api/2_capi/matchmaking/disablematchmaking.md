...
<%= data.branding.codePrefix %>:getMatchMakingService():disableMatchMaking(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.match_making_service.disable_match_making()
