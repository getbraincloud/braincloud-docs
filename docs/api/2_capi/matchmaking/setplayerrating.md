...
<%= data.branding.codePrefix %>:getMatchMakingService():setPlayerRating(playerRating, callback)
```
...
```gdscript
var player_rating = 100

var result = await <%= data.branding.codePrefix %>.match_making_service.set_player_rating(player_rating)
