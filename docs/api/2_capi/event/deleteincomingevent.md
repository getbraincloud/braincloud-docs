...
<%= data.branding.codePrefix %>:getEventService():deleteIncomingEvent(evId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.event_service.delete_incoming_event(ev_id)
