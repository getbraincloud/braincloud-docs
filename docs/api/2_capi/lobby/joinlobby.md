...
<%= data.branding.codePrefix %>.LobbyService.JoinLobby(lobbyId, otherUserCxIds, isReady, extraJson, teamCode, successCallback, failureCallback);
```
...
```cpp
const char *lobbyId = "55555:4v4:3";
...
<%= data.branding.codePrefix %>->getLobbyService()->joinLobby(lobbyId, otherUserCxIds, isReady, extraJson, teamCode, this);
```
...
[[<%= data.branding.codePrefix %> lobbyService] joinLobby:lobbyId
