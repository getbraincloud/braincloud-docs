...
<%= data.branding.codePrefix %>:getGroupFileService():getFileInfo(groupId, fileId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.group_file_service.get_file_info(group_id, file_id)
