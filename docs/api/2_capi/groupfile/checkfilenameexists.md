...
ServerResponse result = await <%= data.branding.codePrefix %>.groupFileService.checkFilenameExists(groupId:groupId, folderPath:folderPath, filename:filename);
...
<%= data.branding.codePrefix %>:getGroupFileService():checkFilenameExists(groupId, folderPath, filename, callback)
```
