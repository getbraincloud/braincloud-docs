...
ServerResponse result = await <%= data.branding.codePrefix %>.globalFileV3Service.getGlobalCDNUrl(fileId:fileId);
...
<%= data.branding.codePrefix %>:getGlobalFileV3Service():getGlobalCDNUrl(fileId, callback)
```
