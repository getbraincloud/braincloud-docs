...
<%= data.branding.codePrefix %>:getEntityService():getEntitiesByType(entityType, callback)
```
...
var result = await <%= data.branding.codePrefix %>.entity_service.get_entities_by_type(entity_type)
