...
ServerResponse result = await <%= data.branding.codePrefix %>.entityService.deleteEntity(entityId:entityId, version:version);
...
<%= data.branding.codePrefix %>:getEntityService():deleteEntity(entityId, version, callback)
```
...
var result = await <%= data.branding.codePrefix %>.entity_service.delete_entity(entity_id, version)
