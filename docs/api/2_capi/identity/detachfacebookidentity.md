...
<%= data.branding.codePrefix %>:getIdentityService():detachFacebookIdentity(facebookId, continueAnon, callback)
```
...
var result = await <%= data.branding.codePrefix %>.identity_service.detach_facebook_identity(facebook_id, continue_anon)
