...
<%= data.branding.codePrefix %>:getIdentityService():attachParseIdentity(parseId, authenticationToken, callback)
```
...
var result = await <%= data.branding.codePrefix %>.identity_service.attach_parse_identity(parse_id, authentication_token)
