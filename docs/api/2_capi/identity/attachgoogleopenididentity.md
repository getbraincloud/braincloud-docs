...
ServerResponse result = await <%= data.branding.codePrefix %>.identityService.attachGoogleOpenIdIdentity(googleId:googleId, authenticationToken:authenticationToken);
...
<%= data.branding.codePrefix %>:getIdentityService():attachGoogleOpenIdIdentity(googleId, authenticationToken, callback)
```
...
var result = await <%= data.branding.codePrefix %>.identity_service.attach_google_open_id_identity(google_id, authentication_token)
