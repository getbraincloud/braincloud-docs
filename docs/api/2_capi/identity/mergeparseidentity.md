...
ServerResponse result = await <%= data.branding.codePrefix %>.identityService.mergeParseIdentity(parseId:parseId, authenticationToken:authenticationToken);
...
<%= data.branding.codePrefix %>:getIdentityService():mergeParseIdentity(parseId, authenticationToken, callback)
```
...
var result = await <%= data.branding.codePrefix %>.identity_service.merge_parse_identity(parse_id, authentication_token)
