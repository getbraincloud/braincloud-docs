...
ServerResponse result = await <%= data.branding.codePrefix %>.identityService.mergeEmailIdentity(email:email, password:password);
...
<%= data.branding.codePrefix %>:getIdentityService():mergeEmailIdentity(email, password, callback)
```
...
var result = await <%= data.branding.codePrefix %>.identity_service.merge_email_identity(email, password)
