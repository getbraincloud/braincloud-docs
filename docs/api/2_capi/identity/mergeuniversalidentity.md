...
<%= data.branding.codePrefix %>:getIdentityService():mergeUniversalIdentity(userId, password, callback)
```
...
var result = await <%= data.branding.codePrefix %>.identity_service.merge_universal_identity(user_id, password)
