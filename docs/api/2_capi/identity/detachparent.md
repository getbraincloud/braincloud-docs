...
ServerResponse result = await <%= data.branding.codePrefix %>.identityService.detachParent();
...
<%= data.branding.codePrefix %>:getIdentityService():detachParent(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.identity_service.detach_parent()
