...
ServerResponse result = await <%= data.branding.codePrefix %>.identityService.getExpiredIdentities();
...
<%= data.branding.codePrefix %>:getIdentityService():getExpiredIdentities(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.identity_service.get_expired_identities()
