...
<%= data.branding.codePrefix %>:getIdentityService():mergeGoogleIdentity(googleId, authenticationToken, callback)
```
...
var result = await <%= data.branding.codePrefix %>.identity_service.merge_google_identity(google_id, authentication_token)
