...
ServerResponse result = await <%= data.branding.codePrefix %>.identityService.detachGoogleOpenIdIdentity(googleId:googleId, continueAnon:continueAnon);
...
<%= data.branding.codePrefix %>:getIdentityService():detachGoogleOpenIdIdentity(googleId, continueAnon, callback)
```
...
var result = await <%= data.branding.codePrefix %>.identity_service.detach_google_open_id_identity(google_id, continue_anon)
