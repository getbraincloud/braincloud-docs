...
<%= data.branding.codePrefix %>:getIdentityService():mergeFacebookIdentity(facebookId, authenticationToken, callback)
```
...
```gdscript
var facebook_id = "someId"
var authentication_token = "someToken"

var result = await <%= data.branding.codePrefix %>.identity_service.merge_facebook_identity(facebook_id, authentication_token)
