...
<%= data.branding.codePrefix %>:getRttService():registerRTTChatCallback(rttCallback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.rtt_service.register_rtt_chat_callback(rttCallback)
