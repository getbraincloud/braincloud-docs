...
ServerResponse result = await <%= data.branding.codePrefix %>.customEntityService.getCount(entityType:entityType, whereJson:whereJson);
...
<%= data.branding.codePrefix %>:getCustomEntityService():getCount(entityType, whereJson, callback)
```
