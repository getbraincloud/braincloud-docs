...
[[<%= data.branding.codePrefix %> playerStateService] getAttributes:successBlock
...
<%= data.branding.codePrefix %>:getPlayerStateService():getAttributes(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.player_state_service.get_attributes()
