...
ServerResponse result = await <%= data.branding.codePrefix %>.gamificationService.readCompletedQuests(includeMetaData:includeMetaData);
...
<%= data.branding.codePrefix %>:getGamificationService():readCompletedQuests(includeMetaData, callback)
```
...
var result = await <%= data.branding.codePrefix %>.gamification_service.read_completed_quests(include_meta_data)
