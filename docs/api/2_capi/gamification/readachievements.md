...
ServerResponse result = await <%= data.branding.codePrefix %>.gamificationService.readAchievements(includeMetaData:includeMetaData);
...
<%= data.branding.codePrefix %>:getGamificationService():readAchievements(includeMetaData, callback)
```
...
var result = await <%= data.branding.codePrefix %>.gamification_service.read_achievements(include_meta_data)
