...
<%= data.branding.codePrefix %>:getGamificationService():readMilestonesByCategory(category, includeMetaData, callback)
```
...
var result = await <%= data.branding.codePrefix %>.gamification_service.read_milestones_by_category(category, include_meta_data)
