...
ServerResponse result = await <%= data.branding.codePrefix %>.gamificationService.readCompletedMilestones(includeMetaData:includeMetaData);
...
<%= data.branding.codePrefix %>:getGamificationService():readCompletedMilestones(includeMetaData, callback)
```
