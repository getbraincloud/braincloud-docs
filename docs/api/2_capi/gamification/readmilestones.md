...
<%= data.branding.codePrefix %>:getGamificationService():readMilestones(includeMetaData, callback)
```
...
var result = await <%= data.branding.codePrefix %>.gamification_service.read_milestones(include_meta_data)
