...
<%= data.branding.codePrefix %>:getGamificationService():readQuestsWithStatus(includeMetaData, callback)
```
...
var result = await <%= data.branding.codePrefix %>.gamification_service.read_quests_with_status(include_meta_data)
