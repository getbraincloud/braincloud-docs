...
ServerResponse result = await <%= data.branding.codePrefix %>.gamificationService.readInProgressQuests(includeMetaData:includeMetaData);
...
<%= data.branding.codePrefix %>:getGamificationService():readInProgressQuests(includeMetaData, callback)
```
...
var result = await <%= data.branding.codePrefix %>.gamification_service.read_in_progress_quests(include_meta_data)
