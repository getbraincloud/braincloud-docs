...
ServerResponse result = await <%= data.branding.codePrefix %>.gamificationService.readAllGamification(includeMetaData:includeMetaData);
...
<%= data.branding.codePrefix %>:getGamificationService():readAllGamification(includeMetaData, callback)
```
...
var result = await <%= data.branding.codePrefix %>.gamification_service.read_all_gamification(include_meta_data)
