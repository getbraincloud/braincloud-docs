...
ServerResponse result = await <%= data.branding.codePrefix %>.gamificationService.readInProgressMilestones(includeMetaData:includeMetaData);
...
<%= data.branding.codePrefix %>:getGamificationService():readInProgressMilestones(includeMetaData, callback)
```
