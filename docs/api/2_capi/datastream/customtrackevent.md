...
ServerResponse result = await <%= data.branding.codePrefix %>.dataStreamService.customTrackEvent(eventName:eventName, jsonEventProperties:jsonEventProperties);
...
<%= data.branding.codePrefix %>:getDataStreamService():customTrackEvent(eventName, jsonEventProperties, callback)
```
...
var result = await <%= data.branding.codePrefix %>.data_stream_service.custom_track_event(event_name, json_event_properties)
