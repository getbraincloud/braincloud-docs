...
<%= data.branding.codePrefix %>:getAuthenticationService():resetUniversalIdPassword(universalId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.authentication_service.reset_universal_id_password(universal_id)
