...
[[<%= data.branding.codePrefix %> playerStatisticsService] readAllUserStats:successBlock
...
<%= data.branding.codePrefix %>:getPlayerStatisticsService():readAllUserStats(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.player_statistics_service.read_all_user_stats()
