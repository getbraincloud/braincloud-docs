...
<%= data.branding.codePrefix %>:getPlayerStatisticsService():resetAllUserStats(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.player_statistics_service.reset_all_user_stats()
