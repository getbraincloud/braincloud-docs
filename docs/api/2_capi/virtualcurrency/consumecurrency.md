...
<%= data.branding.codePrefix %>:getVirtualCurrencyService():consumeCurrency(vcId, vcAmount, callback)
```
...
var result = await <%= data.branding.codePrefix %>.virtual_currency_service.consume_currency(vc_id, vc_amount)
