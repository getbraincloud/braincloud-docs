...
ServerResponse result = await <%= data.branding.codePrefix %>.globalEntityService.getPageOffset(context:context, pageOffset:pageOffset);
...
<%= data.branding.codePrefix %>:getGlobalEntityService():getPageOffset(context, pageOffset, callback)
```
...
var result = await <%= data.branding.codePrefix %>.global_entity_service.get_page_offset(context, page_offset)
