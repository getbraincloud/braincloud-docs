...
<%= data.branding.codePrefix %>:getGroupService():readGroupData(groupId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.group_service.read_group_data(group_id)
