...
ServerResponse result = await <%= data.branding.codePrefix %>.groupService.readGroupMembers(groupId:groupId);
...
<%= data.branding.codePrefix %>:getGroupService():readGroupMembers(groupId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.group_service.read_group_members(group_id)
