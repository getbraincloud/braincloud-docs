...
<%= data.branding.codePrefix %>:getGroupService():approveGroupJoinRequest(groupId, profileId, role, attributes, callback)
```
...
var result = await <%= data.branding.codePrefix %>.group_service.approve_group_join_request(group_id, profile_id, role, attributes)
