...
ServerResponse result = await <%= data.branding.codePrefix %>.groupService.listGroupsPageByOffset(context:context, pageOffset:pageOffset);
...
<%= data.branding.codePrefix %>:getGroupService():listGroupsPageByOffset(context, pageOffset, callback)
```
...
var result = await <%= data.branding.codePrefix %>.group_service.list_groups_page_by_offset(context, page_offset)
