...
ServerResponse result = await <%= data.branding.codePrefix %>.leaderboardService.getPlayerScore(leaderboardId:leaderboardId, versionId:versionId);
...
<%= data.branding.codePrefix %>:getLeaderboardService():getPlayerScore(leaderboardId, versionId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.leaderboard_service.get_player_score(leaderboard_id, version_id)
