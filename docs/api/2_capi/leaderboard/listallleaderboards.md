...
<%= data.branding.codePrefix %>:getLeaderboardService():listAllLeaderboards(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.leaderboard_service.list_all_leaderboards()
