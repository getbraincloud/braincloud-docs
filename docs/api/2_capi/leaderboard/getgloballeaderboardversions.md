...
<%= data.branding.codePrefix %>:getLeaderboardService():getGlobalLeaderboardVersions(leaderboardId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.leaderboard_service.get_global_leaderboard_versions(leaderboard_id)
