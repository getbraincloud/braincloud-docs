...
ServerResponse result = await <%= data.branding.codePrefix %>.messagingService.getMessageBoxes();
...
<%= data.branding.codePrefix %>:getMessagingService():getMessageBoxes(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.messaging_service.get_message_boxes()
