...
ServerResponse result = await <%= data.branding.codePrefix %>.asyncMatchService.findMatches();
...
<%= data.branding.codePrefix %>:getAsyncMatchService():findMatches(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.async_match_service.find_matches()
