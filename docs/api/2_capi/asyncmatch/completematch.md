...
<%= data.branding.codePrefix %>:getAsyncMatchService():completeMatch(ownerId, matchId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.async_match_service.complete_match(owner_id, match_id)
