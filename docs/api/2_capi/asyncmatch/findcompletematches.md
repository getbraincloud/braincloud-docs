...
ServerResponse result = await <%= data.branding.codePrefix %>.asyncMatchService.findCompleteMatches();
...
<%= data.branding.codePrefix %>:getAsyncMatchService():findCompleteMatches(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.async_match_service.find_complete_matches()
