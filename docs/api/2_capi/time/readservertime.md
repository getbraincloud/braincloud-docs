...
ServerResponse result = await <%= data.branding.codePrefix %>.timeService.readServerTime();
...
<%= data.branding.codePrefix %>:getTimeService():readServerTime(callback)
```
...
```gdscript
var result = await <%= data.branding.codePrefix %>.time_service.read_server_time()
