...
<%= data.branding.codePrefix %>:getPlaybackStreamService():endStream(playbackStreamId, callback)
```
...
var result = await <%= data.branding.codePrefix %>.playback_stream_service.end_stream(playback_stream_id)
